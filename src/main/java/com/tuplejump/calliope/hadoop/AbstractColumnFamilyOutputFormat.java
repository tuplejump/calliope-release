...
public abstract class AbstractColumnFamilyOutputFormat<K, Y>
    ...
    public static Cassandra.Client createAuthenticatedClient(String host, int port, Configuration conf) throws Exception {
        ...
        TTransport transport = ConfigHelper.getClientTransportFactory(conf).openTransport(new TSocket(host, port), conf);
        TProtocol binaryProtocol = new org.apache.cassandra.thrift.TBinaryProtocol(transport, true, true);
