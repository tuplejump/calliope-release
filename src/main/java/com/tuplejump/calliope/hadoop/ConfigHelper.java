...
public class ConfigHelper {
    ...
    private static String thriftToString(TBase object) {
        ...
        TSerializer serializer = new TSerializer(new org.apache.cassandra.thrift.TBinaryProtocol.Factory());
        try {
    ...
    private static SlicePredicate predicateFromString(String st) {
        assert st != null;
        TDeserializer deserializer = new TDeserializer(new org.apache.cassandra.thrift.TBinaryProtocol.Factory());
    ...
    public static Cassandra.Client createConnection(Configuration conf, String host, Integer port)
            throws IOException {
        try {
            TTransport transport = getClientTransportFactory(conf).openTransport(new TSocket(host, port), conf);
            return new Cassandra.Client(new org.apache.cassandra.thrift.TBinaryProtocol(transport, true, true));
        } catch (Exception e) {
    ...
    public static ITransportFactory getClientTransportFactory(Configuration conf) {
        ITransportFactory factory = new TFramedTransportFactory();
        return factory;
    }
